...
| Late chunking | Embedding a long window with a long-context model first, then pooling token embeddings into per-chunk vectors that retain whole-document context. | Backlog 6 — the condition is **not met**: the embed model (bge-small-en-v1.5) exposes no token-level embeddings and caps at 512 tokens (see the model decision). |
...
| HNSW | A graph-based ANN index of layered proximity graphs searched greedily from sparse top layers down, giving fast high-recall search. | Phase 3 — the index the [load stage](stages/load.md) creates; explained in the [vector-indexes chapter](theory/vector-indexes.md). |
| IVF (inverted file index) | An ANN index that clusters vectors into partitions and scans only the partitions nearest the query. | Theory — [vector-indexes chapter](theory/vector-indexes.md) (Phase 3), contrasted via pgvector's `ivfflat`; HNSW is the recorded choice. |
| Vector quantization (PQ, SQ, binary) | Compressing stored vectors (product/scalar/binary quantization) to cut index memory at some recall cost. | Theory — [vector-indexes chapter](theory/vector-indexes.md) (Phase 3); the MVP corpus is far too small to need it. Not to be confused with LLM weight quantization (below). |
