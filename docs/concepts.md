...
| Document layout analysis (DLA) | Detecting the visual structure of a page — headings, columns, paragraphs, tables, footnotes — so each region is treated according to its role. | Backlog 12 (named in the item). Phase 1's theory chapter (`corpus-and-parsing`, pinned in the [fetch & convert plan](plans/plan-fetch-convert.md)) contrasts it with lossless XML parsing. |
...
| Text cleaning & normalization | Stripping non-content noise (boilerplate, navigation, editorial apparatus) and normalizing encoding so only meaningful text reaches chunking. | Phase 1 — convert emits normative text only; the exact exclusions (footnotes, editorial apparatus, the XML's own tables of contents) are pinned in the [fetch & convert plan](plans/plan-fetch-convert.md). |
...
| Data lineage | Recording where each piece of indexed data came from and through which transformations, so any chunk traces back to its exact source and fetch time. | Phases 1–3 — source URL and fetch date flow from front matter through chunk metadata into the database row; the concept is named in Phase 1's `corpus-and-parsing` chapter when it lands. |
